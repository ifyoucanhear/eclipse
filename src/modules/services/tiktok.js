...
export default async function (obj) {
    try {
        if (!obj.postId) {
            let html = await got.get(`https://vt.tiktok.com/${obj.id}`, {
                headers: {
            ...
            html.on('error', (err) => {
                return {
                    error: loc(obj.lang, 'ErrorCantConnectToServiceAPI', 'tiktok')
                };
            });
        ...
        if (html.includes(',"preloadList":[{"url":"')) {
            ...
            return {
                error: loc(obj.lang, 'ErrorEmptyDownload')
            };
        }
    } catch (e) {
        return {
            error: loc(obj.lang, 'ErrorBadFetch')
        };
    }
}